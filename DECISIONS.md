...
## Q142 — interactive/adopt-self-authored-skills — interpreted-ambiguity

**Question:** The 30 skills in `~/github.com/soulmachine/skills` were adopted unevenly across the fleet — 6/8/9/14/17/21/23 of 30. Adopt all of them everywhere, or treat the per-host sets as curation?
**Options considered:** leave it, the sets are deliberate / adopt the full set on every host and automate it in the skills role / adopt only the skills whose subject matches the host
**Chosen:** Adopted all 30 on all seven hosts (112 adoptions), and added the adopt step to `roles/skills` so it stays that way.
**Decided-by:** claude
**Justification:** Treated as neglect rather than curation, on evidence: **every host's clone sits on the same commit `258100f`**, so all 30 directories were already on disk everywhere and the only thing missing was the `adopt` call — a curated set would look like divergent clones or divergent content, not divergent link counts against identical trees. The distribution reads as accident too: mac-mini-m2, the always-on host the fleet is driven from, lacked `sync-claude-accounts` and `refresh-claude-account`, which are fleet-credential tools most useful precisely there. And the cost of being wrong is asymmetric — a skill is an instruction file loaded on demand by its description, so an unused one costs nothing, while an absent one silently fails to exist when needed. **This does not reopen Q82.** That decision keeps `soulmachine/skills` out of `skills_sources` because `skills add` would replace each git-backed symlink with a CLI-managed copy; `adopt` is a different verb — a path inside a git repo becomes a Sourced entry, a symlink out to the repo — so the truth stays in git, which is what Q82 was protecting. Checked before running: `--dry-run` confirms the Sourced mechanic, `adopt` is idempotent (an already-Sourced path prints "already Sourced … nothing to do" and exits 0), and **no host had a name collision** between the 30 and a real store directory, so nothing could be clobbered. The role task therefore loops all 30 rather than diffing first, and takes its `changed` verdict from the store's link set before and after — never from `adopt`'s stdout, which says "nothing to do" on every converged run and would report a change forever, the same rule the fingerprint below it follows. Gated on a `stat` of the clone, with a `debug` rather than a failure when it is absent, since a host that does not carry the repo is a legitimate state. `agent-sync` added to the role's `meta/main.yml`, because the task shells out to it. Verified: standalone run reports **0 CHANGED, 0 failed** on a converged host, `--check` exits 0 with no undefined-variable error (both `find` tasks run under check mode, only the `command` is skipped), and ansible-lint passes the production profile. **Two of my own errors are worth recording, because both produced confident wrong output.** First, the verification loop in the rollout script used zsh's `${d:h:t}` on a trailing-slash glob; `:h` behaves like `dirname`, so `/…/skills/foo/` became `/…/skills` and every name evaluated to the literal string `skills` — the script reported `0/30` linked immediately after successfully adopting 13, and only a direct `ls -l` showed the work had landed. Second, the first fan-out ran `ssh host 'zsh script'` and reported **30 failures on five of six hosts**; that is this repo's own documented login-shell trap — Homebrew is put on PATH by `~/.zprofile`, which only a login shell sources, so `agent-sync` was simply not found. Re-running through `zsh -lc` succeeded on all five with zero failures. macbook-pro-nickel passed either way, because its `~/.zshenv` carries the Homebrew path.
**Outcome:** applied
**Ref:** 2529d62

## Q143 — interactive/openclaw-farms-resurrected — gate-resolution

**Question:** Q134 removed the dead OpenClaw symlink farms from six hosts on 2026-09-18, keeping archs-mac-mini on the user's instruction. They are now back on all six, 114 links each. What put them there, and should they be removed again?
**Options considered:** delete them again / find the cause first / leave them and revisit
**Chosen:** Found the cause and **left them in place**, raising it rather than repeating the deletion.
**Decided-by:** claude
**Justification:** Deleting again would have restarted a loop, because the cause is structural and is **this repo**: `roles/skills` installs with `npx skills add … --agent '*'` (and `--all`, which implies the same), the skills CLI counts OpenClaw among the agents it fans out to, and it `mkdir`s the directory it links into. So every run of the skills role recreates `~/.openclaw/skills` on every host — the nightly sweep would have undone any manual removal by the next morning, exactly as it undid Q134's. The attribution is mine and is dated rather than inferred: on mac-mini-m2 the directory's mtime is 02:17:51 with `typesafe-ai` and `unslop` links written at 02:18, matching this session's role run, and on macbook-air and mac-studio-m3 the newest link is `typesafe-ai` at 02:31/02:32, matching the Q141 rollout minutes earlier. This also retires the "unidentified fleet-wide actor" framing carried in project memory for the earlier vanishing-directories sighting — at least for this half, there is no mystery actor. `agent-sync` is a second, independent source of the same effect: its compiled-in registry still lists openclaw as a target (`registry.rs:336`, `Skills::FanOut(".openclaw/skills")`), and `agent-sync status` reports `openclaw (.openclaw/skills) 114 linked` alongside a `missing openclaw-diagnose — not linked yet`. So a durable removal needs both tools to stop treating OpenClaw as an installed agent, not another `rm -rf`; in the skills role that means giving up the `--agent '*'` fan-out or naming agents explicitly, which is a wider change than this question. The farms are inert in the meantime — symlinks only, no OpenClaw binary on any host since Q52-Q54 — so the cost of leaving them is noise rather than risk, and Q142's 112 adoptions added links to directories the role was already repopulating nightly regardless.
**Outcome:** raised
**Ref:** 2529d62

## Q144 — interactive/graphify-instruction-scoped — deviation

**Question:** `~/.agents/AGENTS.md` tells every host "Questions go to the graph first… run `graphify query` before grepping the vault", and `~/.claude/CLAUDE.md` routes `/graphify` to an installed skill. Both files are fleet-wide. The capability exists on two of seven hosts. Provision it everywhere, or scope the instruction?
**Options considered:** provision graphify fleet-wide (CLI + a built graph per host) / scope the instruction to the hosts that have it / leave it
**Chosen:** Scoped the instruction on all seven hosts, on the user's explicit call ("scope the instruction, don't chase the capability").
**Decided-by:** user
**Justification:** Measured on all seven before proposing anything: the `/graphify` skill, the `graphify` CLI and `graphify-out/graph.json` are **all three present only on mac-mini-m2 and macbook-air** and all three absent on the other five, and archs-mac-mini and macbook-pro-nickel carry no `llm-wiki` checkout at all — so on five hosts an agent obeying the instruction runs a command that does not exist. Provisioning was the expensive branch and not a one-liner: the CLI is a `uv tool` install of `graphifyy` (cheap), but `graphify-out/` is **gitignored — 28 MB, zero files tracked** — so the graph is a local build artifact that can never travel with the vault, and making queries work on five hosts means five graph builds plus two vault clones. Same shape as Q140's nickel Chrome rule: a fleet-synced instruction true on one machine, which is the precedent for scoping rather than chasing. **Applied surgically rather than by pushing one file.** mac-mini-m2's copy is 222 lines against 187 on the other six, ahead by an unsynced `cliproxy-key-routing` section, so a whole-file push would have propagated someone else's in-flight work; instead one anchored paragraph was replaced in place on each host, with a dated backup, and every other divergence left alone — verified as exactly 8 added lines per host and the note present once on all seven. The replacement script is idempotent (exits "already scoped" if the note is present) and refuses to touch a file whose anchor does not appear exactly once. The note names the two hosts that have it, records why the graph does not propagate, and ends with an explicit instruction **not** to install the CLI or build a graph to satisfy the section — the failure mode being an agent that reads "questions go to the graph first" and sets about creating one.
**Outcome:** applied
**Ref:** b787782

## Q145 — interactive/agent-sync-1-1-0-reinstall-verified — gate-resolution

**Question:** agent-sync 1.1.0 released and the user routed the fleet reinstall to the agent-sync session. Record that session's report as the outcome, or verify it independently from here?
**Options considered:** accept the peer's per-host report / re-run the reinstall from this session / stand down on running it and verify the resulting state independently
**Chosen:** Stood down from running it, verified the end state independently on all seven hosts, and proved the mechanism end to end.
**Decided-by:** claude
**Justification:** Two sessions upgrading the same seven hosts concurrently is the real hazard, so the reinstall was left where the user routed it — but a peer's report is not evidence, and **verifying it twice already paid off in this work**: two of my own code claims about Gemini's loader were wrong and were corrected by the peer, and one of the peer's branches (legacy-symlink pruning at `~/.gemini/GEMINI.md`) would have destroyed claude-mem's context block fleet-wide had it not been doubly guarded. Independent probe on all seven: `agent-sync --version` 1.1.0, `agent-sync status` exit 0, `context.fileName` = `["GEMINI.md", "../.agents/AGENTS.md"]` with GEMINI.md at index 0 (the element agent-sync must never occupy, since it is Gemini's own `save_memory` write target), the opencode `instructions` entry still the absolute path it already carried — correctly untouched on all three distinct homes (`frankdai`, `developer`, `archiebot`, `lume`) — `~/.codex/AGENTS.md` still a symlink, and **`~/.gemini/GEMINI.md` and `~/.config/opencode/AGENTS.md` still REAL files, not symlinks**, which is the specific regression that would have re-opened the claude-mem write-through hazard. Every claim held. **Then proved delivery rather than configuration**, which nobody had done: the deployed 0.46.0 bundle exports `setGeminiMdFilename` and `getGlobalMemoryPaths`, so both were driven directly against the real home and the real `settings.json` on mac-studio-m3 and archs-mac-mini — each resolved `~/.gemini/GEMINI.md` and `~/.agents/AGENTS.md` as global memory paths, and the loaded Commons carries the Q144 graphify note. That closes the loop from "the config was written" to "the instructions actually reach the model", which an exit code alone does not show. Nothing was hand-applied and nothing remains so: the Gemini leg is agent-sync's to own, which is why it was worth waiting for the release rather than writing the setting by hand when it was first proven.
**Outcome:** applied
**Ref:** 2f1b700

## Q146 — interactive/skills-explicit-agent-list — deviation

**Question:** `roles/skills` installs with `--agent '*'` (and `--all`, which implies it). Q143 left the resulting OpenClaw farm resurrection open, believing a durable fix needed agent-sync to change too. Does it?
**Options considered:** wait for an agent-sync change / name the agents explicitly in this role / keep deleting the directories
**Chosen:** Named the agents explicitly — `universal`, `claude-code`, `pi`, `hermes-agent` — in both install tasks.
**Decided-by:** claude
**Justification:** **Q143's premise was wrong and this corrects it.** agent-sync is not a second source: its `Agent::detected()` is `root_path(home).is_dir()`, so it fans out only into config roots that **already exist** and never creates one — verified against released 1.1.0, which does still list openclaw in its registry. The sole creator is this role: `'*'` is not "every installed agent" but **every agent in the CLI's registry** (`targetAgents = validAgents`), 78 of them as of 2026-09-18, and the installer `mkdir`s each directory it links into. So one change here is sufficient; no upstream change, no repeated `rm -rf`. **`universal` is load-bearing**: it IS the store (`.agents/skills`) and its `detectInstalled` is hardcoded `false`, so it is reached only by being named or by `'*'` — omit it and the store stops being populated, breaking both the symlink farms and the agents that read it natively. The flag is repeated per agent rather than comma-joined, measured rather than assumed: `--agent a,b` parses as the single name `a,b` ("Invalid agents: a,b") while repeating it yields two ("Invalid agents: a, b"). `--all` had to be restated as `--skill '*'` plus the agent list, since it is shorthand for `--skill '*' --agent '*' -y`. This also ends the perpetual `eve` and `promptscript` failures — both declare `globalSkillsDir: void 0`, so a `-g` install has no target — which is why the report task's failure list is now empty rather than permanently two entries long. **The user's archs-mac-mini exception survives untouched and for free:** `~/.openclaw` still exists there, so agent-sync keeps that farm populated by detection. Verified on mac-mini-m2: `~/.openclaw/skills` mtime unchanged at 02:34 across two full role runs that previously rewrote it every time, store still 163 with claude/pi/hermes farms at 167/163/164, **zero dangling links in any of the five directories**, and three consecutive playbook runs at `changed=0 failed=0`. **Not done:** removing `~/.openclaw` from the other six hosts, which is the irreversible half and is the user's call; until then the inert farms stay, they are simply no longer rewritten.
**Outcome:** applied
**Ref:** 2a2835f

## Q147 — interactive/adopt-keys-on-skill-md — deviation

**Question:** The Q142 adopt loop iterates every directory in `~/github.com/soulmachine/skills`. That now fails the whole play on `herdr-advisor`. Tolerate the error, or change what gets adopted?
**Options considered:** `failed_when: false` plus a report task, as the partial-install task does / adopt only directories that are actually skills / special-case the one name
**Chosen:** Key the loop on `SKILL.md` rather than on directories.
**Decided-by:** claude
**Justification:** Found by testing Q146, not reported — the role run failed and the cause turned out to be unrelated to the agent list. A directory is not a skill: upstream commit `5b3d509` "herdr-advisor: moved to OpenSWE/herdr-advisor" moved that skill to its own repo, and **`git` does not delete directories**, so an empty `herdr-advisor/.claude` husk stayed behind. Adopting the husk asks agent-sync to repoint the Commons away from where the skill now really lives, which it correctly refuses: `error: skills/herdr-advisor is already Sourced from …/OpenSWE/herdr-advisor/skills/herdr-advisor — remove the Commons link first if you mean to repoint it`, exit 1, play over. Q142's recorded check that "no host had a name collision" was true when written and stopped being true when the skill moved — which is the general hazard with a loop over whatever a third-party repo happens to contain. Tolerating the error was rejected because it would also swallow a **real** collision between two same-named skills, which is ambiguity a human must resolve rather than a husk to skip; keying on `SKILL.md` skips husks and non-skill directories (docs, CI config) by construction while leaving a genuine collision loud. Implemented as `find` with `patterns: SKILL.md`, `depth: 2`, adopting `item.path | dirname`. 29 skills adopt where 30 directories were tried; the husk is simply not a skill.
**Outcome:** applied
**Ref:** 2a2835f

## Q148 — interactive/openclaw-husk-removed-from-six — gate-resolution

**Question:** With Q146 stopping the recreation, remove `~/.openclaw` from the six hosts that do not run OpenClaw?
**Options considered:** leave the inert farms / remove on all seven / remove on six, keeping archs-mac-mini
**Chosen:** Removed on six, on the user's instruction; archs-mac-mini untouched.
**Decided-by:** user
**Justification:** This is Q134's removal made durable — it failed to stick the first time only because `--agent '*'` recreated the directory on the next play, which Q146 fixed at the source. Surveyed before deleting rather than trusting the premise, and **the survey is the reason this entry exists**: on the six, `~/.openclaw` held `skills/` and nothing else — 146-163 entries, **zero real files, zero links pointing outside the Commons, 0 bytes** — a pure husk whose removal destroys nothing, since every link's target lives in `~/.agents/skills`. On archs-mac-mini the same probe found **6.3 GB, 56,407 real files**, credentials, session transcripts and a live install across 30-plus top-level entries. The user's exception is not a preference to be tidied away; it is the one host where this directory is real. **The removal script guarded itself on content rather than on the host list**, refusing any directory containing a real file, a link outside the Commons, or any top-level entry other than `skills` — so a mis-addressed ssh could not destroy the live install. **That guard was load-bearing, because the guard I would have trusted was silently broken.** A hostname check written as `[[ "$h" == (#i)*arch* ]]` never fired: zsh's `(#i)` case-insensitivity flag requires `EXTENDED_GLOB`, and the script set only `NULL_GLOB`, so the pattern failed to match `archs-mac-mini` — verified afterwards by running all three forms side by side. archs-mac-mini was saved by the content guard alone, and the observable evidence at the time was misleading: the refusal message printed was the content one, which reads like success. Two independent guards on different properties is what made a silent failure a non-event rather than a 6.3 GB loss. Verified after: `~/.openclaw` absent on all six and intact on archs-mac-mini, `agent-sync status` exit 0 everywhere with openclaw no longer reported on the six and still reported on archs-mac-mini (it detects by root existence, so removing the root is what retires it), every store and Claude farm at its prior count, and **zero broken symlinks on any host** — confirming the links were leaves, not targets. The removed link lists are saved per host at `/tmp/openclaw-removed-<host>.txt`.
**Outcome:** applied
**Ref:** 057beef

## Q149 — interactive/skills-role-runs-agent-sync — deviation

**Question:** Installing and adopting fills the store, but no role ever fans it out. `roles/agent-sync` taps, trusts and installs the binary and stops. Should provisioning run `agent-sync sync`?
**Options considered:** leave it to the nightly sweep and to whoever remembers / run `sync` in `roles/agent-sync` right after install / run it at the end of `roles/skills`, once the store is final
**Chosen:** Added it to the end of `roles/skills`, gated on `agent-sync status`.
**Decided-by:** claude
**Justification:** The gap was invisible because the fleet is converged: fan-out is currently correct on all seven only because people and sessions ran `sync` by hand. A **rebuilt** host would install agent-sync and never invoke it, leaving the symlink farms unreconciled and the whole instruction wiring — codex's `AGENTS.md` symlink, opencode's `instructions` entry, gemini's `context.fileName` (Q139, Q145) — simply absent, with nothing failing to say so. This repo's own AGENTS.md already says to run `sync` "after adding or removing any skill"; no role did. Placed at the end of `roles/skills` rather than in `roles/agent-sync` because the store is not final until the installs and the Q142 adopt loop have run, and `roles/skills` already depends on `agent-sync` through its `meta/main.yml`, so the ordering holds for a single-role run too. **Gated on `status`'s exit code, which IS the state diff** — 0 when converged, non-zero with pending work — rather than on `sync`'s stdout, which prints the same reassuring lines whether or not it changed anything and would report a phantom change forever; the same rule this repo applies to `claude update`, `codex update` and `skills update`. The probe takes `failed_when: false`, not `ignore_errors`, since an unconverged host is an expected outcome rather than an error, and `check_mode: false` so the gate still means something in a dry run. **Both branches were exercised, not just the no-op one:** on a converged host the probe is `ok` and the sync `skipping` at `changed=0`; deleting one farm link (`~/.claude/skills/affix-domains`) drove `status` to rc 2, after which the role fired the sync, reported `changed=1`, and restored the link to `../../.agents/skills/affix-domains`; the following run returned to `changed=0` and `--check` passed clean. Testing only the converged path would have proven nothing — a gate that never fires looks identical to a gate that works.
**Outcome:** applied
**Ref:** ba0080d

## Q150 — interactive/unpinned-skills-removed-from-m2 — deviation

**Question:** The user asked to tidy away the 15 unpinned skills present only on mac-mini-m2. Remove all 15?
**Options considered:** remove all 15 / remove all but the ones the user's own instruction files name / leave them
**Chosen:** Removed 14; held `graphify` back.
**Decided-by:** claude
**Justification:** `graphify` is not an experiment like the others: `~/.claude/CLAUDE.md` opens with it, naming the skill **by path** (`~/.claude/skills/graphify/SKILL.md`) and instructing every agent to use it "before doing anything else" when the user types `/graphify`. Removing it would break a documented trigger and falsify the Q144 note this session propagated to all seven hosts stating that mac-mini-m2 has it — a coupled change to two instruction files, not a tidy-up. The other 14 are referenced nowhere: `openroutine`'s two mentions are to the scheduler and its tasks, never the skill, and it was only a symlink into an intact repo. Verified after: 0 dangling links in any of the five farms, store 163 -> 149, `/graphify` still resolves. **An error is recorded here because it cost content.** The intended safety net was a tarball of all 15 taken before deletion; it was written in the same compound command as the `rm`, and it silently produced an EMPTY archive. Cause: the names were held in a shell variable and passed unquoted as `tar czf $A $names` — **zsh does not word-split unquoted parameters**, unlike bash, so `tar` received one bogus filename rather than fifteen; its complaint went to `2>/dev/null`, which the same line had suppressed. The command even printed `contents: 0 top-level entries` and deletion proceeded regardless, because nothing gated the `rm` on the archive being non-empty. There were no APFS local snapshots to fall back on. Of the 14, nine keep a real copy elsewhere on disk — `openroutine`, `paseo`, `paseo-advisor`, `paseo-committee`, `paseo-handoff` and `domain-name-brainstormer` in their own repos, `paseo-help` and `paseo-plugin` in `~/.codex/skills`, `playwright-cli` in `~/.claude/skills` (all confirmed intact after the sync). **Four have no copy anywhere on this host**: `relationship-compatibility-analysis` and the three `source-command-cux-*` skills; `collab-canvas`'s skill content is gone though its `~/.local/bin/collab-canvas` CLI remains. Those five are recoverable, if at all, only from the network Time Machine destination. The empty tarball was deleted rather than left in place, since a 0-entry file named `skills-retired-*.tar.gz` reads as a backup and is worse than none. **The lesson is the ordering, not the quoting:** a destructive step must be gated on its safety net actually containing something, and the safety step is the last place to silence stderr.
**Outcome:** applied
**Ref:** 23e180b

## Q151 — interactive/skills-role-asserts-instruction-wiring — deviation

**Question:** Q149 made the fan-out reproducible, but nothing asserts the result. `agent-sync status`'s exit code cannot: of its seven instruction states, only `missing`, `import` and `legacy` are "actionable" and lift the code to 2 — `conflict` and `foreign` exit **0**. Should this repo verify the per-agent instruction wiring independently, and how deep?
**Options considered:** nothing, Q149's `status` gate is enough / hand-written per-agent checks (codex's symlink target, opencode's `instructions` entry, gemini's `context.fileName` with GEMINI.md at index 0) / reuse `agent-sync status --json` and treat the non-actionable states as failures / drive each agent's own loader to prove delivery
**Chosen:** Reuse `agent-sync status --json` as the surveyor; fail the play on any instruction state outside an allowlist of `linked`/`imported`. Detector in `roles/skills`, `fail` in `main.yml` post_tasks.
**Decided-by:** claude (user chose fail-over-report, config-depth-over-delivery, and fail-under-`--check`)
**Justification:** The hole is measured, not theoretical. Inducing it on mac-mini-m2 — moving the Commons to index 0 of gemini's `context.fileName` — produced `conflict gemini — the Commons is first in context.fileName, where the agent writes its own memories` and **rc 0**, so Q149's `rc != 0` gate skipped the sync and the play went green with gemini loading nothing. Upstream pins this deliberately (`tests/instructions.rs`, `a_conflict_does_not_keep_the_machine_permanently_dirty`), and it is not repairable by re-running: `instructions::apply()` ends `_ => Ok(())`, commented "Conflicts and Foreign links are no-ops". So this is a **detector, not a second repair**, and the failure message is a hand-fix pointing at plain `agent-sync status` — the `--json` document carries only `agent`/`state`/`actionable`, no path and no reason. **Not reimplemented per agent:** agent-sync already owns the registry naming each mechanism, and a hand-written copy here would go stale the moment one changed; this repo already cites that registry as the authority for agent detection. **The allowlist is fail-closed** — a state agent-sync adds in a future release lands outside it and fails loudly, rather than slipping through a blocklist written before it existed, the same posture as the `mac_is_always_on` battery gate. Scoped to the `instructions` array alone: `foreign` occurs on this fleet only under `mcp` and `hooks` (xapi, serena, plannotator — "not in the Commons — left alone"), and the fan-out families have their own `VariantIdentical` state that would turn a deliberate per-agent skill variant into a nightly failure. **rc is ignored on purpose**, not merely tolerated: `status` returns 1 for an unreadable third-party mcp or hooks file while still emitting a complete instructions array, so gating on rc would discard a good survey. **Both branches exercised**, per Q149's rule that a gate which never fires looks identical to one that works: converged → `ok=64 failed=0` with both report tasks and the `fail` skipping; damaged → the report task naming `{"agent": "gemini", "state": "conflict"}` and `failed=1`; restored byte-for-byte against a verified copy and back to green. `--check` passes on a converged host with the survey itself running for real (`check_mode: false`), and deliberately carries no `ansible_check_mode` gate — the repo has never used that variable, and a dry run reporting the host's true state is the point. A reviewer proposed narrowing the predicate with `rejectattr('actionable')` to make `--check` self-correcting; rejected because it would also stop reporting a `missing` that survived the sync, which is exactly the "repair, then fail if still wrong" case.
**Outcome:** applied
**Ref:** 7ec2a7d

## Q152 — interactive/commons-agents-md-provisioned-by-nobody — gate-resolution

**Question:** The Q151 survey returns **no items at all** when `~/.agents/AGENTS.md` is absent, which reads identically to "converged". Nothing creates that file: not agent-sync (no `fs::write` of `commons::INSTRUCTIONS` anywhere in its source; `init` only makes directories) and no role here (`grep -rn 'agents/AGENTS.md' roles/ main.yml` is empty). Should the empty survey fail, and should this repo provision the file?
**Options considered:** fail on an empty survey / report it and continue / add a role that writes a skeleton `~/.agents/AGENTS.md`
**Chosen:** Report it with a `debug` naming the likely cause; do not fail, and do not provision the file here.
**Decided-by:** claude
**Justification:** Failing would make **every freshly built host red forever**, since the Commons instructions file arrives from the openroutine `sync-agentsmd` task rather than from provisioning — a legitimate mid-setup state, not drift, and the same shape as the self-authored-skills report already in this role ("a legitimate state for a host that does not carry them"). Provisioning a skeleton here would give the fleet two writers for one file, and the hourly sync from mac-mini-m2 would overwrite it anyway. The asymmetry is deliberate and is the one place this check is not fail-closed, so it is written down rather than left to be rediscovered. The adjacent catastrophic case stays loud without extra code: if `~/.agents` itself is missing, `status` exits 1 and Q149's ungated `agent-sync sync` fails the play at role 23, well before post_tasks.
**Outcome:** applied
**Ref:** 7ec2a7d

## Q153 — interactive/selfauthored-skills-span-multiple-repos — deviation

**Question:** `~/.agents/AGENTS.md` now describes self-authored skills graduating out of the `soulmachine/skills` monorepo into their own OpenSWE-org repos, and two already have (`herdr-advisor`, `log-decisions`). The Q142/Q147 adopt loop walks exactly one hardcoded path. Should it follow them?
**Options considered:** leave it — the monorepo is the only managed home / glob `~/github.com/OpenSWE/*/skills` / declare an explicit list of skill-parent directories
**Chosen:** `skills_selfauthored_repo` (one path) became `skills_selfauthored_repos` (an explicit list of directories that directly contain skill directories), and `swe-workflow/log-decisions` was dropped from `skills_sources`.
**Decided-by:** claude (user chose clone-over-CLI for the duplicated skill)
**Justification:** Measured 2026-09-18: `herdr-advisor` was a hand-adopted symlink into `~/github.com/OpenSWE/herdr-advisor/skills/herdr-advisor` on **all seven hosts**, was in no `skills_sources` entry and carried no lock entry — so nothing in this repo created it and a rebuilt host would simply not have had it. That is the Q149 gap one level down: correct everywhere only because a human ran `agent-sync adopt`. An explicit list rather than a glob because the repo should state what it manages, and because every entry is then uniformly a directory whose children are skills — one `find` at `depth: 2` covers all three, verified returning 31 skills (29 monorepo + 2 OpenSWE). **`log-decisions` had two mechanisms at once:** the role installed it from `swe-workflow/log-decisions` while six of seven hosts carried the OpenSWE symlink, leaving only mac-mini-m2 with a CLI real directory that a `git pull` cannot reach — exactly what a Sourced symlink exists to prevent. The old source name still resolves (GitHub redirects the org transfer), so this was duplication rather than breakage. Dropping it strands nothing: `agent-sync adopt --dry-run` reports "would replace the identical Commons copy with the link" (it refuses only when contents diverge, and they are byte-identical), and `find-orphans.py` counts **real directories only** — "an entry that is a symlink belongs to agentstow" — so the orphan report that a removed source would otherwise trigger stops as soon as the entry becomes a link. Ordering makes that self-resolving inside one run: the adopt loop sits ~50 lines above the orphan scan. Roots are stat-filtered before `find` rather than passed blind, because a missing clone is a legitimate state on a host that does not carry it and `find` would otherwise warn on every run; the absent roots are named in words instead.
**Outcome:** applied
**Ref:** 8fce774

## Q154 — interactive/marketplace-add-gated-on-the-ledger — bugfix

**Question:** `claude plugin marketplace add` ran unconditionally for all four marketplaces on every play. After the `swe-workflow` org transfer it began failing the play outright. Gate it, migrate the stored declaration on every host, or tolerate the error?
**Options considered:** gate the add on the marketplace name already being in `known_marketplaces.json` / rename the source and `remove` + re-add on all seven hosts / `failed_when` tolerating the "network source differs" line / leave it and report
**Chosen:** Gated the add on the name being absent from the ledger, and corrected the source to `OpenSWE/swe-workflow` for hosts that have not registered it yet.
**Decided-by:** claude (user chose the ledger gate)
**Justification:** **`claude plugin marketplace add` is not idempotent across an upstream org rename**, which is what turned a harmless redundant call into a permanent failure. The evidence is the ledger's own timestamps: the four records were written at 13:08:15 / :19 / :21 / :24Z — four entries, in this loop's order, three seconds apart — so the role's own loop wrote them, and the `swe-workflow/swe-workflow` add followed GitHub's redirect and stored the **resolved** `OpenSWE/swe-workflow`. Every later run then asked for the old spelling, which Claude Code refuses ("its network source differs from the one declared for it in settings … the source must match"). Measured the same day: mac-mini-m2 already declared `OpenSWE/swe-workflow` and failed, while the other six still declared the old name — so they were one run from flipping and two from failing, and a bare rename in the role would have broken those six rather than fixed them. The gate fixes all seven without touching anyone's Claude config, because the ledger is keyed by marketplace **name**, which a source rename does not change. It also restores the rule the rest of this repo follows — do not re-run an installer whose state you can read — and the task was already slurping that exact ledger for its `changed` verdict, so nothing new is read. Rejected the `failed_when` tolerance because it keeps the pointless re-add and makes a genuine add failure read identically to this one. **Verified:** all four items `skipping`, the verdict task printing "already registered", and the play reaching `failed=0` where it had been `failed=1`. The ledger records still move during a play — the plugin install/update path and a live Claude Code session both refresh them — which is why the gate reads the name rather than trusting a timestamp.
**Outcome:** applied
**Ref:** 62649db

## Q155 — interactive/adopt-tolerates-a-lagging-clone — bugfix

**Question:** `agent-sync adopt` exits 1 with "already Sourced from … remove the Commons link first" when a host's `soulmachine/skills` clone still carries a skill that has since graduated to its own repo. Measured 2026-09-18, six of seven hosts are in that state. Should that fail the play?
**Options considered:** leave it failing — a duplicate name is ambiguity / tolerate that one refusal and report it / have the role `git pull` the monorepo clone / de-duplicate by name and prefer the OpenSWE root
**Chosen:** Tolerate that one refusal, report it with the `git pull` remedy, and keep every other adopt failure fatal through a new `fail` in post_tasks.
**Decided-by:** claude
**Justification:** **This was already broken before Q153 and I mis-scoped it.** Q147 keyed the adopt loop on `SKILL.md` to skip the empty husk `git mv` left behind, and I recorded the husk as isolated to mac-mini-m2 — true of the *husk*, and irrelevant to the six hosts whose clone predates the move and therefore still holds a real `herdr-advisor/SKILL.md`. Measured: m2 is at `5b3d509`, all six others at `111ac0b`, and `agent-sync adopt --dry-run` on the stale copy exits **1** on every one of them. So the skills role has been failing fleet-wide since the graduation, and the nightly sweep's `failed=0` test was catching it while I read the single-host husk as the whole story. The Commons link is already correct on all seven — it resolves to `OpenSWE/herdr-advisor` — so the refusal reports a clone wanting a `git pull`, not a decision needing a human, and failing the play leaves the 20 roles after `skills` unrun over a link that is right. Rejected pulling the clone from the role: it is the user's working repo, carries uncommitted skill edits, and no role has ever managed it. Rejected preferring a root by rule, which would silently pick a winner and hide the lag. The sentinel is **positive** — "remove the Commons link first" — because a negated match would swallow every other failure mode including the genuine same-name collision, which still routes to `fail`; the same-path no-op prints "nothing to do" and exits 0, so it never reaches the classifier. Classification happens after the loop rather than in `failed_when` because a looped `failed_when` cannot see the item's own `rc` in ansible-core 2.21 — "'rc' is undefined", measured, not assumed — which is the same constraint that shaped the pinned-skill install. `selectattr('rc', 'defined')` guards `--check`, where the command is skipped and the result carries no `rc`.
**Outcome:** applied
**Ref:** e8b8933

## Q156 — interactive/typesafe-role-scope — gate-resolution

**Question:** The ask was "a role named typesafe to install all tools based on the TypeSafe Jev model", with one upstream doc linked (fast-jev-compaction). "All tools" is open-ended: the ecosystem also holds a community MCP plugin for Claude Code, pi extensions, and MCP servers for Codex. Which set is the role's?
**Options considered:** fast-jev-compaction only / fast-jev-compaction + the claude-jev MCP plugin / every agent this repo provisions (pi-typesafe, pi-jev, typesafe-mcp for Codex as well)
**Chosen:** The two Claude Code plugins: `fast-jev-compaction@fast-jev-compaction` (function-hook compaction) and `claude-jev@claude-jev` (MCP, five `jev_*` tools). The official `typesafe-ai` *skill* is not added — `roles/skills` already pins it (Q141).
**Decided-by:** human
...
**Question:** How does `TYPESAFE_API_KEY` reach the two plugins, and what does the role do on a host that has no key?
**Options considered:** the plugins' own `sensitive` userConfig via `claude plugin install --config apiKey=…` (keychain) / `settings.json` `env` map, as upstream's install doc shows / the `dotenv` role's `~/.zshenv` export alone / install the plugins regardless of the key
**Chosen:** Write `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` and `TYPESAFE_API_KEY` into `~/.claude/settings.json`'s `env` map (read-merge-write, `recursive=true`, `no_log` on the two tasks that carry the value), before the plugin install; gate the whole role on the key being non-empty, reporting why when it is not.
**Decided-by:** agent
...
**Question:** The `claude-code` role writes `~/.claude/settings.json` with `mode: "0644"`. Once the file carries an API key, is that still right?
**Options considered:** leave 0644 / 0600 in both roles / 0600 in the typesafe role only
**Chosen:** 0600 in both roles.
**Decided-by:** agent
...
## Q159 — interactive/typesafe-key-fleet-rollout — irreversible-action

**Question:** After Q156–Q158 the key existed only in mac-mini-m2's `.env`, so the other six hosts would have taken the no-key branch tonight and landed the plugins on the *second* nightly sweep (the dotenv role rewrites `~/.zshenv` inside the same play the typesafe role reads the ambient environment in). Wait for that, or put the key on every host now?
**Options considered:** wait two nightly sweeps / append the key to each host's `.env` and let tonight's sweep do the rest / append the key, then run the dotenv and typesafe roles on each host now
**Chosen:** The third: appended the line to all six `.env` files over ssh, with the value piped on stdin rather than placed on any argv, then ran the dotenv role and — in a fresh login shell, the sweep's own form — the typesafe role on each.
**Decided-by:** agent
**Justification:** The user answered "what's next" twice after the plugins were listed as blocked on this one step, and a TypeSafe key is not host-scoped the way the tailnet-scoped Tailscale auth key is, so the same value belongs in every `.env` and the per-machine divergence CLAUDE.md protects does not apply. Cheap to reverse (delete one line per host) and no data was overwritten: each append was gated on the key being absent and `.env` was left at 0600. Before the rollout the no-key branch was exercised on m2 with the variable blanked — it reported and installed nothing, per Q149's rule that a gate which never fires looks identical to one that works. After it, every host reads: both marketplaces registered, both plugins installed and enabled, both env keys present, `settings.json` at 0600, no `pluginConfigs` written. Idempotency and delivery were then proven on macbook-air rather than on the host the role was developed on, since m2 is the sync source and cannot show lag: a second run reported 0 CHANGED, a `jev_ask` call through `claude-gw` returned a typed probability from `jev-1.13.0`, and that session's debug log shows the fast-jev hooks module registered on `session.compact` and `turn.complete` and the claude-jev MCP server connected. The extra CHANGED on mac-mini-2018 was the claude-code dependency moving it from 2.1.276 to 2.1.277, which the other five had already taken.
**Outcome:** applied
**Ref:** f7b5db3

## Q160 — interactive/mini-brew-cache-prune — irreversible-action

**Question:** Last night's sweep on mac-mini-m2 died in the homebrew role: three cask downloads (codex, chatgpt, cmux) failed with curl's "Failure writing output to destination". The data volume was at 100% with 4.2 GiB free and `~/Library/Caches/Homebrew/downloads` held 6.0 GiB. Free the space now, unattended, or report it?
**Options considered:** report only / `brew cleanup --prune=all` now and pre-fetch the three casks / also delete other caches (Google 3.4 GiB, ms-playwright 2.1 GiB, codex 1.4 GiB)
**Chosen:** `brew cleanup --prune=all`, then `brew fetch --cask codex chatgpt cmux` to prove the download step passes; nothing outside Homebrew's own cache was touched.
**Decided-by:** agent
...
**Question:** The cmux role taps `manaflow-ai/cmux` but never trusts it, while the four sibling tap roles (cc-switch, moshi, multica-cli, agent-sync) each carry a guarded `brew trust`. Measured 2026-09-18: the tap is absent from `trust.json` on macbook-air, macbook-pro-nickel and dev-server-frank-lume, yet last night's sweep upgraded cmux to 0.64.25 on all three. Is the missing step a bug?
**Options considered:** leave it — the fleet is converged / add the same guarded trust step as cc-switch / trust every untrusted tap fleet-wide in the homebrew role
**Chosen:** Add the guarded trust step to the cmux role, in the cc-switch shape: read `brew trust --json v1`, run `brew trust --tap` only when the tap is absent from the ledger.
**Decided-by:** agent
...
## Q162 — interactive/sweep-run-capped-by-idle-timer — gate-resolution

**Question:** Why did the 2026-09-18 02:00 sweep leave no report, and is the cause structural? The Run record reads `timed-out`, the daemon log says `went quiet for too long; killed the process group` at 02:16:48, and the Run's output holds only the header and Claude Code's connector warning. The three previous nights succeeded in 6.5, 12.4 and 6.8 minutes.
**Options considered:** treat it as a one-off from the full disk / raise `idle_timeout` in the daemon's config.toml / switch the task's agent to a streaming output mode so the pulse keeps beating / change openroutine to count something other than stdout as activity
**Chosen:** — (report; no config changed)
**Decided-by:** agent (to escalate)
**Justification:** Two causes, one acute and one structural, and only the acute one is settled. Acute: the daemon log carries six `No space left on device` errors between 02:03:01 and 02:03:26 that night — it could not create run directories for two other tasks or write its own schedule file — so the sweep agent, which had already launched the seven hosts, went silent on a disk with zero bytes free and was killed at the quiet ceiling; Q160 removed that cause. Structural: openroutine's pulse is `run::Activity`, stamped only by reads from the child's combined stdout/stderr pipe (`capture_output` in src/run.rs, `await_child` in src/daemon.rs), and `claude -p` writes nothing to that pipe between its opening warning and its final answer — tool output and Monitor events stay inside the agent's context. So for this agent the "idle" budget is in practice a **hard cap on total Run time of 15 minutes**, the runbook's "that printing is load-bearing" paragraph cannot help, and the three green nights were green because they finished under the cap. The same ceiling killed `x-timeline-domain-hunter` twice on 09-18 (18:18 and 19:18, each 15 minutes after its start). Per-task override is refused by design (src/task.rs: "`idle_timeout` is set machine-wide in config.toml, not per task"), so every remedy is either a machine-wide change to the user's own daemon or a change to its agent contract — the user's product decision, not a provisioning one. Nothing in this repo is wrong; the play itself was fine on the hosts that ran it.
**Outcome:** escalated
**Ref:** b09275f

## Q163 — interactive/mini-uv-cache-prune-blocked — gate-resolution

**Question:** With the mini's disk the cause of last night's failure, `~/.cache/uv` holds 49 GiB (45 GiB of unpacked wheel archives). `uv cache prune` — the tool's own command for dangling entries — timed out after 300 s waiting for the cache lock. Force it?
**Options considered:** `uv cache prune --force` / kill the lock holders first / leave it
**Chosen:** Leave it; report the lock holders.
**Decided-by:** agent
**Justification:** `lsof` names the holders: three `uvx … pyright-langserver` processes (Claude Code's Python LSP for open sessions, one alive since 09-15) and `uvx … chroma-mcp` (claude-mem's vector store, feeding every session on this host). `uvx` keeps the lock fd open for the life of the tool it launched, and `prune` also removes cached environments — the `environments-v2` tree those four processes are running from — so `--force` would ignore exactly the in-use check that protects them. Not catastrophic, but a live memory store is not something to yank unattended. The urgency is also gone: the data volume read 112 GiB free before the prune started, up from 9.4 GiB eight hours earlier, and nothing this session did accounts for more than the 6.6 GiB of Q160 — no local Time Machine snapshots exist now, so either the user, a peer session, or APFS purgeable reclamation freed the rest; the cause is unverified and stated as such. Both brew and uv caches regrow, so the number worth watching is the trend, not tonight's.
**Outcome:** applied
**Ref:** b09275f

## Q164 — interactive/q150-restore-abandoned — gate-resolution

**Question:** Q150 left five skills "recoverable, if at all, only from the network Time Machine destination". The user says to ignore it — there is no Time Machine. Close the restore option, and is the recorded loss still accurate?
**Options considered:** close it silently / close it and correct Q150's loss list / attempt some other recovery
**Chosen:** Closed, and the loss list corrected — three of the five were never lost.
**Decided-by:** user (to abandon the restore); agent (to verify the list before recording it as final)
**Justification:** The restore option is gone by the user's decision: no Time Machine destination exists, so the five items Q150 named have no backup to come back from and the question is settled rather than deferred. But recording that as a five-item loss would have been wrong. Measured 2026-09-19: `source-command-cux-status`, `source-command-cux-support` and `source-command-cux-usage-refresh` are all present again in `~/.agents/skills` and `~/.claude/skills`, as **real directories created 09-19 02:03** — during the nightly sweep — and they appear nowhere in `~/.agents/.skill-lock.json`, so the skills CLI did not fetch them. Their owning tool regenerates them: `cux` is installed at `~/.local/share/mise/installs/node/lts/bin/cux`, and the `source-command-*` prefix is how it registers its own slash commands as skills. A tool-generated skill is not data loss at all — it was reconstituted by the next run of the thing that writes it, with no action from anyone. What is genuinely, permanently gone is **two**: `relationship-compatibility-analysis`, which exists nowhere on this host, and `collab-canvas`'s skill content, whose `~/.local/bin/collab-canvas` CLI survives but whose SKILL.md does not. Neither is referenced by any role, instruction file or task here. The Q150 lesson is untouched by this — the empty-tarball ordering bug was real and cost the two that did not regenerate; the correction narrows what it cost, and records that a skill's regenerability, not its backup, is what decided the difference.
**Outcome:** applied
**Ref:** 5f5f325

## Q165 — interactive/typesafe-ai-skill-moves-to-typesafe-role — deviation

**Question:** The ask was to add `typesafe-ai/skills` to the `typesafe` role. That skill is already installed — `roles/skills` pinned it in `skills_sources_partial` (Q141), and Q156 recorded it as deliberately not this role's. Add a second installer, or move the one that exists?
**Options considered:** add the install to `typesafe` and leave the pin in `roles/skills` (two roles refreshing one store directory) / move it: install in `typesafe`, record it in `roles/skills` as foreign / decline as already installed
**Chosen:** Moved. `roles/typesafe` now installs it, and its entry in `roles/skills` went from `skills_sources_partial` to `skills_sources_foreign`.
**Decided-by:** agent
**Justification:** Declining was wrong — the ask is about *which role owns it*, and the answer Q156 gave is the one being revised. Duplicating was worse: two roles would run `skills add` on the same source in one play, and a name that broke upstream would then be reported twice while `find-orphans.py` counted the directory as accounted-for by whichever list still held it. `skills_sources_foreign` exists for exactly this shape and already carries `herdrdev/herdr`, which `roles/herdr` installs for the same reason — the skill documents the tool the role installs. Two calls the ask did not settle, both argued from this repo's own precedent: (1) **the skill sits outside the API-key gate.** The role's gate says a plugin that can never reach Jev is noise, but a SKILL.md is guidance for writing code against TypeSafe and is useful before a key exists, and `roles/skills` installed it unconditionally on all seven hosts — gating it would have silently narrowed where it lands, so only the plugin half still skips and the no-key message was reworded to say so. (2) **a broken pin is `failed_when: false` plus a report, not a play failure.** `typesafe` is role 31 of 44; failing there would stop 13 later roles — `skills`, `herdr`, `hermes`, `ponytail`, `claude-mem` among them — on every nightly sweep until someone edited a line, which is the blast radius `roles/skills/tasks/main.yml` explicitly refuses for its own pins. No `creates:` guard, because re-running the installer is the only thing that pulls upstream edits down (the lesson that role opens with); the verdict is a `find … get_checksum` either side of the call, carried on the second `find` rather than a `debug` whose verdict the `minimal` callback would swallow. The four agents are named rather than `--agent '*'`, mirroring `skills_agents` — which also makes a standalone run self-sufficient, since the `agent-sync sync` that would otherwise fan the store out runs seven roles later. Verified on mac-mini-m2: `ansible-lint` clean on both changed roles, and `ansible localhost -m include_role -a name=typesafe` reported **0 changed** with the installer at rc=0, writing `~/.agents/skills/typesafe-ai` plus symlinks for Claude Code, Pi and Hermes Agent — the same four targets the skills role produced. Upstream still ships exactly one skill (`-l`, 2026-09-19), so the pin filters nothing today and exists for what it does later. Not yet on the fleet: the other six hosts get it from tonight's sweep once this is committed and pushed.
**Outcome:** applied
**Ref:** bb85e6f

## Q166 — interactive/cua-role-permissions-gate — gate-resolution

**Question:** `/plan` asked for a new `cua` role: install `cua-driver` via its curl one-liner, then install `jev-use` per `docs/.../driver/jev-use.mdx`. That doc's own "Step 1: Verify Cua Driver" says permissions readiness is `accessibility`/`screen_recording` booleans that must both read `true` — but running `cua-driver permissions status --json` for real on this host (cua-driver was already installed here) returned a different shape entirely: `{"daemon_running": false, "reason": "no CuaDriver daemon is running under the driver's own identity (com.trycua.driver), so its real TCC status can't be read from this process. Run \`cua-driver permissions grant\` to grant + verify.", "status": "unknown"}`. How should the role decide "ready," and should it try to grant the OS permissions itself?
**Options considered:** gate on the doc's stated `accessibility`/`screen_recording` fields (never matches what the tool actually emits) / gate on the observed `status` field, treating anything but `"granted"` as not-ready / drive the OS consent flow unattended (launch the daemon under its own identity, click through Accessibility/Screen Recording prompts) and only then re-check
**Chosen:** Gate on `(permissions status --json).status == "granted"`. Anything else — including the observed `"unknown"` — fails the play with the exact remedy the tool itself names (`cua-driver permissions grant`) plus the raw JSON, and does not touch the git-clone/`uv sync` steps. No attempt to launch the daemon or drive the consent dialogs from the role.
**Decided-by:** agent
**Justification:** The doc's assumed schema is unusable as written — real output has no `accessibility`/`screen_recording` keys at all when the daemon isn't running under its own launchd identity; `status: "unknown"` is the only field verified against live output, so `"granted"` is inferred by symmetry with the tool's own enum-style summary field, not copied from a granted-state sample (none was reachable in this session — granting requires clicking through real macOS TCC dialogs). Auto-driving those dialogs was rejected the same way the `tailscale` role rejects it for its Network Extension toggle (`CLAUDE.md` — Tailscale role specifics): `ansible.builtin.pause` and any scripted click-through degrade silently or hang non-interactively, so the existing house pattern is fail loud with the literal remedy command, which this role now matches. Also decided in the same pass, same root question of where "install" ends: the doc's Step 1 additionally lists Chrome-installed and screen-unlocked as preconditions, but those gate *running the live example*, not installing it — a provisioning run (including over SSH, screen locked) can legitimately precede either, so the role does not check them. Verified live on Franks-MacBook-Air: `~/.local/bin/cua-driver` already present (installer's `creates:` skip confirmed idempotent), `ansible-lint` clean, `ansible localhost -m include_role -a name=cua --check` correctly skips the already-installed binary, reads real status, and fails with this exact message since permissions aren't granted on this host yet.
**Outcome:** applied
**Ref:** 2dfdc87

## Q167 — cua role — deviation

**Question:** Fanning the `cua` role out to the fleet exposed that `cua-driver permissions status --json` does not have one stable schema, as Q167's predecessor (Q166) assumed. On mac-mini-m2, mac-mini-2018, mac-studio-m3, archs-mac-mini and macbook-air (daemon not running under its own identity, `com.trycua.driver`) it reports `{"daemon_running": false, "reason": "...", "status": "unknown"}`, matching what Q166 was based on. But on dev-server-frank-lume — where the daemon *is* running and permissions are genuinely granted — the real output carries no top-level `status` field at all: `{"accessibility": true, "screen_recording": true, "direct_capture_error": null, "direct_capture_status": "not_checked", "screen_recording_capturable": null, "source": {...}}`. Gating on `.status == "granted"` (Q166's design) read the missing `status` key as `"unknown"` and reported lume as *not* granted even though it plainly was — a false negative on the one host that was actually ready to proceed past the gate. How should the gate be fixed to handle both observed shapes correctly?
**Options considered:** keep the `status`-enum gate and special-case the second shape / gate directly on `accessibility` and `screen_recording` booleans, defaulting each to `false` when absent / probe the daemon's running state first and branch the whole task list on it
**Chosen:** Gate on `(status.accessibility | default(false)) and (status.screen_recording | default(false))` directly, dropping the `status`-enum check entirely. This correctly falls through to "not granted" when those keys are absent (the daemon-not-running shape) and correctly reads the real booleans when the daemon is running (both true on lume, so the gate now passes there).
**Decided-by:** agent
**Justification:** A single boolean check subsumes both observed shapes without branching on which one is in play — no need to first probe `daemon_running` separately, since a missing key already defaults safely to `false`. This directly matches the doc's own original description of the readiness condition (`accessibility`/`screen_recording` both `true`), which Q166 had discarded as "unusable as written" from only having seen the daemon-not-running shape; the fleet run supplied the missing granted-state sample and showed the doc's original field names were right all along — only the enum-based `status` gate Q166 substituted for them was wrong.
**Outcome:** applied
**Ref:** (pending)
**Supersedes:** Q166 — gate design corrected after real granted-state output from dev-server-frank-lume contradicted the `status`-enum assumption

## Q168 — fleet-github-pat — deviation

**Question:** Rolling one all-scopes classic PAT out to the fleet, `setup_github.sh` refused to run on every host: `Error: GITHUB_TOKEN is missing scopes: user:email read:user` — against a token that demonstrably had both capabilities (`gh api /user/emails` and `gh api /user --jq .name` both returned data). Work around it per host, or fix the check?
**Options considered:** export the two child scopes onto the token (impossible — GitHub's UI offers `user` as one checkbox and reports only what was granted) / bypass the check for this rollout / fix the scope comparison in the script
**Chosen:** Fixed the comparison. `x-oauth-scopes` lists only *granted* scopes, never the children a parent implies, so a PAT holding `user` reports `user` and not `user:email`/`read:user`. Added a `has_scope()` helper that splits the header on commas, strips spaces/tabs, and matches whole scopes with `grep -qx`; `user:email` and `read:user` are additionally satisfied by `user`.
**Decided-by:** agent
**Justification:** This was not a token problem — it was a gate that no all-scopes token could ever pass, which means the `github` role has been inert on **every** host for as long as the fleet PAT has been the four-scope one: the role's `rescue:` block catches the non-zero exit and downgrades it to a debug message, so each nightly sweep printed a warning and moved on while SSH key registration silently never happened. That is why `mac-mini-m2`'s ed25519 key was registered as a *signing* key but not an *auth* key, and why `dev-server-frank` had no signing config at all. Fixing the check is the root-cause fix; bypassing it would have left the next host to rediscover the same wall. The rewrite also closes a latent false-positive the old `grep -qF "$s"` carried: `user:email` contains the substring `user`, so a token granted only the child would have passed a requirement for the parent. A five-case self-check (parent-only, children-only, the old under-scoped PAT, the substring trap, and the live header) drove out one bug in the fix itself before it shipped — the first draft used `tr -d '[:space:]'`, which deletes newlines too and collapsed the candidate list into a single unmatchable line, failing closed on every input.
**Outcome:** applied
**Ref:** (pending)

## Q169 — fleet-github-pat — gate-resolution

**Question:** The fleet's shared classic PAT had only `gist, read:org, repo, workflow`. Replacing it raised four calls the ask did not settle: which hosts are in scope, what expiry, what to do about `archs-mac-mini` (a different GitHub identity), and whether to prune account keys matching no live host.
**Options considered:** per-host tokens / one shared PAT with a bounded expiry / one shared PAT with no expiry; include or exclude `archs-mac-mini`; prune orphaned keys or leave them
**Chosen:** One classic PAT, all 21 scopes, **no expiration**, on eight hosts (`mac-mini-m2`, `macbook-air`, `mac-mini-2018`, `mac-studio-m3`, `dev-server-frank-lume`, `supermicro`, `dev-server-frank`, and `macbook-pro-nickel` when it returns). `archs-mac-mini` excluded and its copy of the soulmachine token to be stripped. Seven orphaned account keys deleted. `windows-zx8` untouched.
**Decided-by:** user (all four calls, via plan review); agent (execution order and the pre-delete proof below)
**Justification:** The shared-PAT architecture already existed — all eight reachable hosts carried the identical `ghp_U7qQ…` — so this was a scope-and-hygiene fix, not a re-architecture. No expiry was chosen against the agent's recommendation of one year, on the reasoning that unattended jobs (`openroutine`'s hourly `x-timeline-domain-hunter`) must not break silently; the tradeoff is recorded rather than argued again. `archs-mac-mini` is genuinely a different identity (`ArchAutoTech` / `tech@archauto.com`, its own `id_rsa` on no soulmachine list), so running `setup_github.sh` there would have registered its key on the wrong account and rewritten its git identity — but its `GITHUB_TOKEN` sits at `~/.zshenv:31`, *outside* the managed block and absent from its `.env`, and with that line gone `gh auth status` reports its ArchAutoTech keyring credential already invalid. Stripping therefore has to be preceded by a real `gh auth login` there, which needs a browser and is left to the user. Two ordering constraints were forced by the data rather than chosen: **`supermicro` had no ed25519 key at all**, and `setup_github.sh` only generates one when *no* key of any type exists — it has `id_rsa`, so without generating first the script would have re-registered the RSA key that Q169 was about to delete; and its RSA key was its **only** registered credential, so deletion had to wait on proof. Before removing either RSA entry, `ssh -o IdentitiesOnly=yes -i ~/.ssh/id_ed25519 -T git@github.com` was run on both `supermicro` and `mac-mini-m2` and authenticated on the new key alone. The orphan set was re-derived from live data (every host's actual `*.pub` diffed against both account lists) rather than from the inventory taken during planning; that rerun correctly flagged `windows-zx8` and `macbook-pro-nickel` as unmatched *only* because those hosts were unreachable, and both were retained. `macbook-air` was a second trap: its `GITHUB_TOKEN` lived **inside** the dotenv-managed block but had gone missing from its `.env`, so the ordinary "edit `.env`, re-run the role" path would have regenerated the block without it and silently unset the token on that host. End state verified on all seven reachable hosts: new token live in a non-interactive shell, `gh api /user` → `soulmachine`, `admin:public_key` + `admin:ssh_signing_key` present, `ssh -T` authenticated, and a real empty commit verifying as `Good "git" signature`. Account now holds 9 auth and 9 signing keys — one pair per host, no duplicates. The old PAT is **not yet revoked**: `macbook-pro-nickel` is offline and still holds it, so revocation is gated on that host being reprovisioned.
**Outcome:** applied
**Ref:** (pending)

## Q170 — fleet-github-pat — gate-resolution

**Question:** Q169 gated revocation of the old PAT on `macbook-pro-nickel`, which was offline. It came back 2026-09-22. Does anything still depend on the old token?
**Options considered:** revoke immediately / sweep for remaining copies first, then revoke
**Chosen:** Swept first. `macbook-pro-nickel` is now on the new token with both key types registered and a verified signed commit, so all eight in-scope hosts are complete. A fleet-wide grep for the old token's literal value across `~/.zshenv`, `~/.zshrc`, `~/.zprofile`, `~/.bashrc`, `~/.profile` and each `.env` finds it in exactly one file: `archs-mac-mini:~/.zshenv`. Revocation is therefore safe for every in-scope host and is now gated solely on Q169's archs-mac-mini step, not on this repo.
**Decided-by:** agent
**Justification:** `macbook-pro-nickel` turned out to be a third layout, distinct from both the managed-block hosts and the no-checkout Linux ones: its `GITHUB_TOKEN` was a hand-written `export` at `~/.zshenv:35` — *after* the managed block (which ends at line 29) — and absent from its `.env`. Simply adding it to `.env` and re-running `dotenv` would have looked like it worked and done nothing: the block regenerates at line ~29 and the stray line at 35 re-exports the old value afterwards, last assignment winning. So the stray line was deleted (guarded on `line > block_end`, `zsh -n` validated) and the token added to `.env`, which normalises the host onto the same convention as the rest of the fleet. That makes three distinct pre-existing layouts found across nine hosts for one variable — inside the block (five hosts), outside it (`macbook-pro-nickel`, `archs-mac-mini`), and no checkout at all (`supermicro`, `dev-server-frank`) — which is the argument for the rotation procedure now written into `~/.agents/AGENTS.md` rather than left as folklore. Deliberately **not** done here: removing the dead token line from `archs-mac-mini`. Revoking the PAT kills `gh` on that host either way, but Q169 requires a working `ArchAutoTech` credential to be restored there first, and that needs an interactive browser login the agent cannot perform.
**Outcome:** applied
**Ref:** (pending)
