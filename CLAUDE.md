...
- **Two GUI-only steps are gated, and they FAIL the play rather than prompt.** `ansible.builtin.pause` degrades badly on a non-interactive stdin — Ansible only warns ("Not waiting for response to prompt") and continues, so the play used to barrel past the gate and die several tasks later on something unrelated-looking (`tailscale up` reporting `No such file or directory: b'tailscale'`). Both gates now fail at the point of detection with the actual remedy, which behaves identically interactively and headless. Re-run the playbook after doing the step.
  ...
  - **CLI launcher.** Gated on `stat /usr/local/bin/tailscale`, wrapped in a block gated on `(auth key set) or (OAuth configured)`. Both credential paths shell out to the CLI — `tailscale up` for enrolment, `tailscale status --json` for the device ID the API calls need — so either one arms the gate, and with neither the role still converges on the cask install alone. It used to sit *inside* the `TAILSCALE_AUTH_KEY` block, which is what coupled key-expiry to the auth key; see below.
...
- **Key-expiry disable takes an OAuth client and nothing else.** When `TAILSCALE_OAUTH_CLIENT_ID` + `TAILSCALE_OAUTH_CLIENT_SECRET` are both set, the role form-`POST`s them to `/api/v2/oauth/token` (no `grant_type` parameter; Tailscale's own documented curl omits it), then reads `Self.ID` from `tailscale status --json` (the REST API accepts it as the device identifier) and `POST`s to `/api/v2/device/{id}/key` with `keyExpiryDisabled: true`. The returned bearer token lives exactly one hour and that is not configurable — measured, `expires_in: 3600`. The client needs only the **`devices:core`** scope, whose documented endpoint list names `POST /api/v2/device/:deviceID/key` directly.
...
- **The two credentials gate different halves of the role, and are deliberately not chained.** `TAILSCALE_AUTH_KEY` enrols the *node* (`tailscale up`, spent once); the OAuth client authenticates *control-plane API calls* (key expiry, ongoing). The key-expiry block is therefore gated on `tailscale_oauth_configured` **alone**. It used to also require `tailscale_auth_key | length > 0` — not by design but by accident of layout: the OAuth facts were loaded 100 lines below the auth-key block, so the only way to reach the CLI check was to nest inside it. The effect was that blanking a spent auth key on a long-enrolled host silently switched off key-expiry maintenance. All credential loading now happens up front, before either is used.

  What the block does still need is a node to act on, so it reads `Self.ID` **first** — before minting a token, so an unenrolled host does not burn a credential it cannot use — and emits a `debug` no-op when there is no ID. A `debug` rather than a `fail`: an unenrolled machine is a legitimate mid-setup state, not drift. Every step of the extraction defaults (`stdout | default('{}', true)`, `.Self | default({}, true)`, `.ID | default('', true)`), because an unenrolled node yields an empty stdout, a null `Self`, or a `Self` with no `ID` depending on how far setup got — all three verified. That read is a *fresh* `tailscale status`, not a reuse of the one in the enrolment block, which on a first run is captured before `tailscale up` and carries no `Self.ID`.
