...
- **Key-expiry disable, and why it prefers an OAuth client.** When an API credential is also set, the role reads `Self.ID` from `tailscale status --json` (Tailscale's REST API accepts this as the device identifier) and `POST`s to `/api/v2/device/{id}/key` with `keyExpiryDisabled: true`. Skipped silently if no credential is set. Two credential shapes are accepted, in this order:
  - **`TAILSCALE_OAUTH_CLIENT_ID` + `TAILSCALE_OAUTH_CLIENT_SECRET`** — preferred. The role form-`POST`s the pair to `/api/v2/oauth/token` (no `grant_type` parameter; Tailscale's own documented curl omits it) and uses the returned bearer token, which lives exactly one hour and is not configurable. The client needs the **`devices:core`** scope — verified against the scopes table, whose endpoint list for that scope names `POST /api/v2/device/:deviceID/key` directly. **Both halves are required**; half a pair is treated as a misconfiguration and does *not* silently fall back to the token below.
  - **`TAILSCALE_API_ACCESS_TOKEN`** — legacy, still honoured so an unmigrated fleet host keeps converging. Personal access tokens carry **no scopes at all** (fully permitted) and **expire 90 days** after minting, which turns this task into a 401 on a timer nobody watches; a `debug` task prints the migration steps on every run that falls back to one. That is deliberately a notice and not a failure — the token works right up until it doesn't.

  An OAuth client secret **cannot** be used as a bearer token directly, hence the exchange. It *can* be passed straight to `tailscale up --auth-key=`, but that needs the `auth_keys` scope and forces tag ownership onto the node, so `TAILSCALE_AUTH_KEY` stays a separate credential.
